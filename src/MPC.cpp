...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // initialize first item of fg into ZERO
    fg[0] = 0;
    ...
    for (unsigned int t = 0; t < N; t++) {
      ...
      fg[0] += 10000 * CppAD::pow(vars[epsi_start + t] - ref_espi, 2);
...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  // assign value from state vector

  // most of parts below are copied from CarND-MPC-Quizzes
  // we have already done sanitary check on pre-simulation.
